...
Output for single neuron:$ \text{neuron_output} = \text{input} \cdot \text{weight} + \text{bias} $

Output for multiple neurons:$\ \text{output} = \sum_{i=0}^{n} (\text{input}_i \cdot \text{weight}_i) + \text{biases} \$
